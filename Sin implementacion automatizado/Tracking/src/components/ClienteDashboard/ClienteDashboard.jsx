...
const ClienteDashboard = ({ user, uid, onLogout }) => {
  ...
  const cargarPaquetes = useCallback(async (cargarMas = false, forzarRecarga = false) => {
    ...
    try {
      ...
      // 🔥 Si la respuesta no es JSON (es HTML), recargar página
      const contentType = response.headers.get('content-type');
      if (contentType && contentType.includes('text/html')) {
        console.log('Servidor reiniciado, recargando página...');
  ...
  const cargarFotos = useCallback(async (trackingId) => {
    ...
    try {
      ...
      // Silencioso, no molestar al usuario por fotos
  ...
  const verTracking = useCallback(async (id, reintento = 0) => {
    ...
    try {
      ...
      if (!response.ok) {
        // 🔥 Si es error 503 (temporal), reintentar hasta 3 veces
        if (response.status === 503 && reintento < 3) {
          console.log(`Error temporal, reintentando... (${reintento + 1}/3)`);
          setTimeout(() => {
            verTracking(id, reintento + 1);
          }, 500 * (reintento + 1));
          return;
        }
        
        if (response.status === 403) {
          // No mostrar alerta si es error temporal de permisos
          if (!data.temporal) {
            alert('No tienes permiso para ver este paquete');
          }
        } else {
      ...
      console.error('Error en verTracking:', error);
      if (reintento < 2) {
        console.log(`Error de red, reintentando... (${reintento + 1}/2)`);
        setTimeout(() => {
          verTracking(id, reintento + 1);
        }, 1000);
      } else {
