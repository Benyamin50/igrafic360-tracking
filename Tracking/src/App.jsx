      ...
      try {
        ...
        if (userData) {
          ...
          if (typeof paquetesArray === 'string') {
            try {
              paquetesArray = JSON.parse(paquetesArray);
            } catch (e) {
              paquetesArray = [];
            }
          }
          ...
          const clienteData = {
            ...userData,
            paquetes: Array.isArray(paquetesArray) ? paquetesArray : []
          };
      ...
      <header className="app-header-vip">
        ...
        {user && (
          <nav className="header-nav">
            <button 
              className={`nav-item ${modo === 'cliente' ? 'active' : ''}`}
            ...
            <button 
              className={`nav-item ${modo === 'rastreo' ? 'active' : ''}`}
              onClick={() => cambiarModo('rastreo')}
            >
              Rastrear
            </button>
            
            <button 
              className={`nav-item ${modo === 'direcciones' ? 'active' : ''}`}
              onClick={() => cambiarModo('direcciones')}
            >
              Direcciones
            </button>

            <button 
              className={`nav-item ${modo === 'reporte' ? 'active' : ''}`}
              onClick={() => cambiarModo('reporte')}
            >
              Reportar paquete
            </button>
            ...
            {userRol === 'admin' && (
              <button 
                className={`nav-item ${modo === 'admin' ? 'active' : ''}`}
          ...
          {user ? (
            <div className="user-badge">
              ...
              <div className="user-info-text">
                ...
                <span className={`user-role ${userRol === 'admin' ? 'role-admin' : ''}`}>
                  {userRol}
                </span>
              </div>
            </div>
      ...
      <main className={getMainClass()}>
        {modo === 'cliente' ? (
            ...
            <div className="rastreo-grid">
              ...
              <div className="left-panel">
                ...
                <p style={{ color: '#A0AEC0', marginBottom: '24px', fontSize: '0.9rem' }}>
                  Ingresa tu numero de guia para ver el estado actual de tu envio.
                </p>
                
                <TrackingSearch 
                  onSearch={handleSearch} 
                  misPaquetes={user?.paquetes || []} 
                  userRol={userRol} 
                />
              </div>
              ...
              <div className="right-panel">
                {loading && (
                  <div className="rastreo-state-box">
                    <div className="spinner-mini" style={{ width: '40px', height: '40px' }}></div> 
                    <p style={{ color: '#D4AF37', marginTop: '16px' }}>Cargando informacion...</p>
                  </div>
                )}
                ...
                {!trackingData && !loading && (
                  <div className="rastreo-state-box">
                    ...
                    <p style={{ color: '#718096', marginTop: '16px' }}>
                      Ingresa un numero de guia para ver los detalles
                    </p>
                  </div>
                )}
                ...
                {trackingData && !loading && (
                  <div className="results-box">
                    ...
                    <div>
                      <h4 style={{ color: '#D4AF37', marginBottom: '16px', fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span>📋</span> Historial del envio
                      </h4>
                      <TrackingTimeline 
                        steps={trackingData}
                        fotosMap={{}}
                        tiposUbicacion={[]}
                        onVerFoto={(url) => window.open(url, '_blank')}
                      />
                    </div>
          ...
          <div style={{ padding: '40px 20px', textAlign: 'center', maxWidth: '600px', margin: '40px auto', background: '#1A202C', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.3)', border: '1px solid #2D3748' }}>
            ...
            <p style={{ color: '#A0AEC0', marginBottom: '30px', fontSize: '1.1rem', lineHeight: '1.6' }}>
              Tienes mucho tiempo esperando tu paquete?
              <strong> Por favor escribenos en nuestras redes</strong> para poder brindarte un buen servicio.
            </p>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <a 
                href="https://wa.me/584125309882" 
                target="_blank" 
                rel="noopener noreferrer" 
                style={{ background: '#25D366', color: 'white', padding: '15px 20px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', transition: '0.2s' }}
              >
                Escribenos por WhatsApp (+58 412-5309882)
              </a>
              
              <a 
                href="mailto:locasio918@gmail.com" 
                style={{ background: '#3182CE', color: 'white', padding: '15px 20px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', transition: '0.2s' }}
              >
                Envianos un Correo (locasio918@gmail.com)
              </a>
            </div>
          </div>
        ) : (
          userRol === 'admin' ? <AdminPanel /> : (
